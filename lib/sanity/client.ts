...
import { allRecipeSlugsQuery } from "./groq/recipeQueries";
import {
  AllPostCategories,
  Recipe,
  SimplePost,
...
export async function getCategorizedPostCategories(limit) {
  if (client) {
    try {
      ...
      const categories: AllPostCategories = await client.fetch(query);
...
export async function getAllRecipeSlugs() {
  if (client) {
    const slugs: Slug[] = await client.fetch(allRecipeSlugsQuery);
